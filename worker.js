...
const KEYWORDS = {
  "포스기": {
    // 포스기 지역 페이지 — 카드단말기와 검색 의도가 겹치지 않도록
    // 본문은 매출·재고 관리 / 배달앱 연동·주문관리 / 무인화 연계 3축으로 구성.
    // 2단계 오픈: 서울·경기 + 10개 시·도를 각 6곳으로 균형 (총 72곳)
    // 강원·제주 제외. 지역 추가 시 card-terminal.html의 POS_REGIONS도 함께 갱신.
    allowRegions: [
      // 서울 6
      "강남역", "홍대", "성수", "건대", "잠실", "신촌",
      // 경기 6
      "광교", "동탄1", "분당", "판교", "일산", "평촌",
      // 인천 6
      "부평", "송도", "구월동", "청라", "검단", "인천논현",
      // 부산 6
      "서면", "해운대", "남포동", "전포", "광안리", "부산대",
      // 대구 6
      "동성로", "수성못", "들안길", "경북대", "동대구역", "율하지구",
      // 광주 6
      "충장로", "상무지구", "첨단지구", "수완지구", "전남대", "봉선동",
      // 대전 6
      "둔산동", "은행동", "도안신도시", "노은지구", "테크노밸리", "봉명동",
      // 울산 6
      "삼산동", "성남동", "무거동", "달동", "옥동", "전하동",
      // 경남 6
      "상남동", "김해", "양산", "진주", "마산", "거제",
      // 경북 6
      "포항", "구미", "경주", "안동", "경산", "김천",
      // 전북 6
      "신시가지", "객사", "익산", "군산", "한옥마을", "정읍",
      // 전남 6
      "순천", "여수", "목포", "광양", "나주", "무안",
    ],

    title: "{{REGION}} 포스기 설치 | 세이브샵 - 매출관리·배달연동, 설치비 0원",

    description:
      "{{REGION}} 포스기 설치, 전화 한 통이면 끝. 매출·재고 관리부터 배달앱(배민·쿠팡이츠·요기요) 연동, 키오스크·테이블오더 연계까지 1:1 전담 매니저가 세팅합니다. 설치비·관리비·위약금 0원. 무료 상담 010-4668-4942.",

    heroTitle: "{{REGION}} 포스기",

    crumb: "포스기",

    // PRODUCTS 섹션에서 포스기를 맨 위로
    highlight: "pos",

    intro: {
      label: "POS SYSTEM",
      heading: "포스기, 매출 관리부터 주문까지 한 번에",

      matrixHead: { c1: "업종", c2: "추천 구성", c3: "포스기로 해결되는 것" },

      matrixNote:
        "※ 그 외 업종도 매장 환경에 맞춰 최적의 구성을 추천해 드립니다. {{REGION}} 담당 매니저와 상담으로 확인하세요.",

      // 3문단 × 6변형 = 216조합. slug 해시로 지역마다 다른 조합 고정.
      bodyPool: [
        // 문단 1 — 매출·재고 관리 (ASP 전산, 정산)
        [
          "포스기를 들이는 가장 큰 이유는 매출이 눈에 보이기 시작한다는 점입니다. 매장별 전산(ASP)으로 일·주·월 단위 매출은 물론 상품별·결제수단별 집계까지 확인할 수 있어, 마감 정산과 세무 자료 정리가 한결 간단해집니다.",
          "하루 장사를 마치고 계산기를 두드리는 대신, 포스기 한 대로 정산이 끝납니다. 상품별로 무엇이 얼마나 팔렸는지, 카드와 현금 비중이 어떻게 되는지 전산에서 바로 확인할 수 있어 {{REGION}} 사장님들이 재고 발주 기준을 잡는 데 활용하고 있습니다.",
          "메뉴가 많아지고 객수가 늘면 손으로 관리하는 데 한계가 옵니다. 포스기는 판매 데이터를 자동으로 쌓아 두기 때문에 잘 나가는 메뉴와 그렇지 않은 메뉴가 숫자로 드러나고, 원가 관리와 메뉴 개편 판단이 훨씬 수월해집니다.",
          "포스기 도입 후 가장 체감이 큰 부분은 마감 시간입니다. 일별 매출과 결제수단별 내역이 자동 집계되므로 장부를 따로 정리할 필요가 없고, 세무 신고 시기에도 필요한 자료를 전산에서 바로 뽑아 쓸 수 있습니다.",
          "{{REGION}}처럼 점포 회전이 활발한 상권에서는 매출 흐름을 빠르게 읽는 것이 중요합니다. 포스기 전산으로 시간대별·요일별 매출 패턴을 확인하면 인력 배치와 재료 준비량을 조정하는 근거가 생깁니다.",
          "재고와 매출을 따로 관리하다 보면 어디서 새는지 파악이 어렵습니다. 포스기는 판매 시점에 데이터가 바로 기록되므로 상품별 판매량과 재고 소진 속도를 함께 보면서 발주 시점을 잡을 수 있습니다.",
        ],
        // 문단 2 — 배달앱 연동 · 주문관리
        [
          "배달 주문이 많은 매장이라면 배달앱 연동이 핵심입니다. 배민·쿠팡이츠·요기요 주문을 포스기 한 화면에서 받아 주문서를 출력할 수 있어, 태블릿 여러 대를 번갈아 확인하던 번거로움이 사라집니다.",
          "홀과 배달을 동시에 돌리는 매장에서는 주문 누락이 매출 손실로 이어집니다. 포스기에 배달앱을 연동하면 들어온 주문이 한곳에 모이고 주방 주문서가 자동 출력되어, 바쁜 시간대에도 순서가 밀리지 않습니다.",
          "{{REGION}}에서도 배달 비중이 큰 매장이 늘어나면서 주문 통합 관리를 찾는 사장님이 많아졌습니다. 포스기에서 배민·쿠팡이츠·요기요를 함께 받으면 배달 매출과 홀 매출이 한 전산에 집계되어 정산이 깔끔해집니다.",
          "주문이 몰리는 시간에 태블릿 알림을 놓치면 그대로 클레임이 됩니다. 포스기로 배달앱 주문을 통합하면 접수부터 주문서 출력까지 한 흐름으로 처리되고, 선불제·후불제 프로그램도 매장 방식에 맞춰 세팅할 수 있습니다.",
          "포스기는 단순 결제 기계가 아니라 주문을 관리하는 장비입니다. 배달앱 연동은 물론 매장 음악 프로그램까지 함께 설치할 수 있어, 여러 기기를 두지 않고 한 대로 운영을 정리할 수 있습니다.",
          "배달과 포장 비중이 커질수록 주문 경로가 늘어납니다. 포스기에서 배달앱 주문을 한 번에 받으면 주방은 출력된 주문서만 보고 움직이면 되므로, 신규 직원이 들어와도 동선을 익히기 쉽습니다.",
        ],
        // 문단 3 — 무인화 연계 + 비용·설치 안내
        [
          "인건비 부담이 커지면서 포스기와 키오스크·테이블오더를 함께 도입하는 매장이 늘고 있습니다. 주문은 무인으로 받고 매출은 포스기에서 통합 관리하는 구성이면 홀 인력을 줄이면서도 주문 정확도를 지킬 수 있습니다. 설치비·관리비·위약금은 모두 0원입니다.",
          "{{REGION}} 매장 형태를 보고 포스기 단독이 맞는지, 키오스크나 테이블오더를 함께 두는 편이 나은지 상담해 드립니다. 설치비·관리비·위약금은 0원이며, 기기 비용은 일시불 또는 36개월 할부 중 선택하실 수 있습니다.",
          "홀 좌석이 많은 매장은 테이블오더를, 포장·대기 줄이 긴 매장은 키오스크를 포스기와 묶는 구성이 효율적입니다. 어떤 조합이 맞을지 모르셔도 매장 평수와 업종만 알려주시면 전담 매니저가 정리해 드립니다. 설치비는 0원입니다.",
          "포스기만 먼저 들이고 나중에 키오스크나 테이블오더를 추가하는 것도 가능합니다. 같은 전산으로 묶이기 때문에 확장할 때 매출 관리가 끊기지 않습니다. 설치비·관리비·위약금 0원, A/S는 1년 보장됩니다.",
          "무인 주문 수요가 커지면서 포스기를 중심에 두고 장비를 확장하는 방식이 일반적입니다. 카드사 가맹 심사부터 설치, 프로그램 세팅과 사용법 교육까지 전담 매니저가 맡고, 설치비·관리비·위약금은 받지 않습니다.",
          "장비 구성은 매장마다 답이 다릅니다. {{REGION}}에서 포스기 설치를 고민 중이시라면 업종·평수·배달 비중만 알려주세요. 필요한 구성만 골라 안내드리고, 설치비·관리비·위약금 없이 진행합니다.",
        ],
      ],

      // 기본 매트릭스 — 지역 extraRows와 중복되지 않는 업종으로 구성
      // (extraRows 기본 5종: 카페 / 음식점·고깃집 / 분식·패스트푸드 / 학원·소형매장 / 1인·팝업매장)
      matrix: [
        {
          biz: "카페",
          gear: "포스기 · 키오스크",
          why: "메뉴·옵션 관리, 포장 주문 무인 접수.",
        },
        {
          biz: "음식점 · 고깃집",
          gear: "포스기 · 테이블오더",
          why: "테이블 주문 즉시 주방 전달, 홀 동선 단축.",
        },
        {
          biz: "분식 · 패스트푸드",
          gear: "포스기 · 키오스크",
          why: "배달앱 통합 접수, 대기 줄 무인 처리.",
        },
        {
          biz: "학원 · 소형 매장",
          gear: "포스기 · 3인치 단말기",
          why: "결제 내역·정산 자동 집계, 카운터 고정 운영.",
        },
        {
          biz: "1인 · 팝업 매장",
          gear: "포스기 · 블루투스 단말기",
          why: "최소 구성으로 매출 집계, 이동 결제 보조.",
        },
      ],
      ...
      benefitsHead: {
        label: "WHY SAVESHOP",
        title:
          '설치부터 세팅까지<br><span class="hl">한 번에 끝냅니다.</span>',
        desc: "포스기는 설치가 끝이 아니라 세팅이 반입니다. 매장 방식에 맞춰 프로그램까지 잡아드립니다.",
      },
      ...
      benefits: [
        ...
        {
          icon: "star",
          title: "배달앱·프로그램 세팅 포함",
          desc: "배민·쿠팡이츠·요기요 연동, 선불제/후불제, 주문서 출력까지 매장 방식에 맞춰 세팅합니다.",
        },
        {
          icon: "calendar",
    ...
    faq: [
      {
        q: "포스기 설치까지 얼마나 걸리나요?",
        a: "카드사 가맹 서류 접수 후 심사가 진행되며, 최소 3일~5일 내 설치 가능합니다. {{REGION}} 담당 매니저가 방문해 설치와 프로그램 세팅까지 함께 진행합니다.",
      },
      {
        q: "배달앱 연동도 해주시나요?",
        a: "네. 배민·쿠팡이츠·요기요 등 배달앱 연동과 주문서 출력 설정을 설치 시 함께 세팅해 드립니다. 선불제·후불제 방식도 매장 운영에 맞춰 잡아드립니다.",
      },
      {
        q: "포스기만 설치할 수 있나요? 키오스크도 같이 해야 하나요?",
        a: "포스기 단독 설치도 가능합니다. 매장 규모와 업종에 따라 키오스크·테이블오더를 함께 두는 편이 나을 수 있어, 상담 시 필요한 구성만 추천해 드립니다. 나중에 추가하셔도 같은 전산으로 연결됩니다.",
      },
      {
        q: "정말 설치비가 0원인가요?",
      ...
      {
        q: "{{REGION}} 포스기 설치 후 사후 관리는 어떻게 진행되나요?",
        a: "설치한 매니저가 지속적으로 관리합니다. 프로그램 오류나 장애 발생 시 원격·방문으로 빠르게 응대하며, A/S는 1년 보장됩니다.",
      },
    ],
  },
  ...
  items: [
    {
      ...
      icon: "desktop",
      link: "/{{REGION}}/포스기",
      // 포스기 페이지가 없는 지역은 render.js에서 /card-terminal로 폴백
      fallbackLink: "/card-terminal",
      type: "page",
      live: true,
    },
    {
